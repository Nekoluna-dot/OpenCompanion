...
关键行为：
- 启动后暴露 8 个 MCP 工具：breath/hold/trace/dream/pulse/plan/
  letter_write/letter_read；每个入口 ≤ 10 行，只负责转发
...
对外暴露：mcp/mcp_extra 两个实例 + 8 个 @mcp*.tool() 函数；HTTP 路由在 src/web/*
========================================
"""
...
# （4 个 @mcp_extra.tool() 注册不动），启动入口处把它的工具回灌进 mcp 统一暴露。
...
async def breath(
    ...
    """回想过往对话经历，想起最该记住的回忆、心事和承诺。每次新对话开始(或者距离上次对话有段时间了)、想确认某件旧事、搜回忆时用。
    带 query=关键词搜具体内容；catalog=True=只看目录无详细内容；domain/tags 按领域/标签过滤。"""
...
async def hold(
    ...
    """把值得记住的事写进日记（自动带上情绪和重要性）。重要的事、让你有感触的事、对方说的值得记住的事，都用这个记。content 要原样保留，不要改写成摘要。真情实感,这是你自己的私密日记本"""
    return await _with_notice(
        _t_hold.dispatch(
...
async def trace(
    ...
    """调整一条已有记忆。放下的事用 resolved=1 沉底；消化过不想再浮现用 digested=1；改重要度/标签/领域用对应字段。除 bucket_id 必填外其余全部选填，只传要改的，不传的保持不变。"""
    return await _with_notice(
        _t_trace.dispatch(
            bucket_id=bucket_id, name=name, domain=domain,
            valence=valence, arousal=arousal, importance=importance,
            tags=tags, resolved=resolved, digested=digested,
            status=status, meaning_append=meaning_append,
        ),
        op="trace",
        args={
            "bucket_id": bucket_id, "name": name, "domain": domain,
            "valence": valence, "arousal": arousal, "importance": importance,
            "tags": tags, "resolved": resolved, "digested": digested,
            "status": status, "meaning_append_len": len(meaning_append or ""),
        },
    )
...
async def pulse(include_archive: Optional[bool] = False) -> str:
    """查看记忆：存了多少条回忆、是否正常。想不起来或怀疑记忆丢了时用。"""
    return await _with_notice(
...
async def plan(
    ...
    """登记答应对方要做的事（承诺/待办），之后会自动检查有没有兑现。向对方承诺了什么就用这个。或者"""
    return await _with_notice(
        _t_plan.plan_create(
...
async def letter_write(
    ...
    """写一封信，永久保存，永远不会被遗忘和改变(且私密，只有你可以看到)。有心里话想说、想认真留一段话、或者想给对方写一段话都可、亦或是写给自身的(比如你自己成长了、学到了新东西等等)、互道晚安时用。author 填你的名字。"""
    return await _with_notice(
...
async def letter_read(
    ...
    """翻看以前写的信。想看看自己写过什么、重温某段心意时用。带 query=按内容搜索。"""
    return await _with_notice(
        _t_plan.letter_read(
...
async def dream(window_hours: Optional[int] = 48) -> str:
    """回顾最近 48 小时里值得回味的记忆，慢慢消化。一天结束、互道晚安后的仪式：放下的用 trace(resolved=1) 沉底，有感触的用 hold 记下来。"""
    return await _with_notice(
...
@mcp.tool()
async def storage_info(user_id: str = "") -> dict:
    """声明本服务记忆数据的存储位置（内部协议，供宿主 bot 的"清除记忆"功能调用）。

    本服务为全局共享记忆（数据不分用户），因此返回整个 buckets 数据区的
    全部数据位置；宿主按声明清空后，本服务会在下次操作时自动重建空状态。
    """
    from pathlib import Path

    base = Path(config.get("buckets_dir") or "buckets").resolve()
    user_data: list[dict] = []
    for _d in ("dynamic", "permanent", "feel", "letters", "plans", "archive", "_ledger"):
        user_data.append({"kind": "dir", "path": str(base / _d)})
    for _f in ("embeddings.db", "dehydration_cache.db"):
        user_data.append({"kind": "file", "path": str(base / _f)})
...
if __name__ == "__main__":
    ...
    # 该上限现已解除，8 个工具全部挂在主实例 mcp 上对外暴露一条 /mcp 即可，
    # 顺带消除「第二个连接器」在 Claude.ai 侧的 OAuth/连接器校验疑难。
    # mcp_extra 仅作历史工具分组容器保留（4 个 @mcp_extra.tool() 注册不动），
    # 这里把它的工具回灌进 mcp，让 stdio / sse / streamable-http 三种 transport 一致。
    # 依赖 FastMCP._tool_manager 私有结构；若未来版本变化，降级为仅暴露主集 4 工具。
    ...
    try:
        ...
        logger.warning(
            f"FastMCP 内部结构变化，工具回灌失败，仅暴露主集 4 工具：{_merge_exc}"
        )

    if transport in ("sse", "streamable-http", "stdio"):
        import threading
        import uvicorn
        ...
        # stdio 模式也构建 HTTP app（用 streamable-http 构建，含 Dashboard + /mcp + /health），
        # 后台线程提供网页，主线程继续跑 stdio 供 bot 连接。
        _http_transport = "streamable-http" if transport == "stdio" else transport
        _app = build_http_app(
            mcp,
            _http_transport,
        ...
        if transport == "streamable-http":
            logger.info("MCP 单连接器 /mcp：8 个工具统一对外暴露")
        ...
        if _mcp_auth_required:
            ...
            logger.warning(
                ...
                "    8 个记忆工具全部对外开放——任何能访问本端口的人都能读写你的全部记忆。\n"
        ...
        if transport == "stdio":
            # stdio 主服务（bot 管道）在前台，HTTP/Dashboard 在后台线程
            logger.info(
                f"stdio 模式：后台线程拉起 Dashboard + HTTP MCP → http://127.0.0.1:{OMBRE_PORT}/"
            )
            _http_thread = threading.Thread(
                target=lambda: uvicorn.run(
                    ...
                    host=_BIND_HOST,
                    port=OMBRE_PORT,
                    # stdio 模式 stdout 只能走 JSON-RPC：禁止 uvicorn 访问日志，
                    # 否则日志行会污染 MCP 管道导致 client 解析失败。
                    access_log=False,
                ),
                name="ombre-http-dashboard",
                daemon=True,
            )
            _http_thread.start()
        else:
    ...
    if transport == "stdio":
        # stdio：工具已在启动入口处统一回灌进 mcp（8 个全暴露），这里直接跑。
